...
public partial class ManageFacultyEvents_admin : System.Web.UI.Page
{
    private string conStr = ConfigurationManager.ConnectionStrings["CGConnection"].ConnectionString;

    protected void Page_Init(object sender, EventArgs e)
    {
        // Ensure ScriptResourceMapping for jquery exists so client-side validators work under UnobtrusiveValidationMode
        try
        {
    ...
    protected void btnCreate_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                con.Open();
                string query = @"INSERT INTO FacultyEvents_tbl
            ...
            ClientScript.RegisterStartupScript(GetType(), "created", "alert('Faculty Event Created Successfully!');", true);
            ClearForm();
            BindEvents();
        }
        catch (Exception ex)
    ...
    private void ClearForm()
    {
        ...
        // file label reset is client-side; the file input can't be cleared reliably server-side
    }
    ...
    private void BindEvents()
    {
        ...
        using (SqlConnection con = new SqlConnection(conStr))
        {
            string query = "SELECT Id, EventName, StartDate, EndDate, Description, FileName FROM FacultyEvents_tbl ORDER BY Id DESC";
    ...
    protected void gvEvents_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(conStr))
            {
                ...
                string sql = @"UPDATE FacultyEvents_tbl
    ...
    protected void gvEvents_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        try
        {
            ...
            {
                con.Open();
                string sql = "DELETE FROM FacultyEvents_tbl WHERE Id=@Id";
                using (SqlCommand cmd = new SqlCommand(sql, con))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
                    cmd.ExecuteNonQuery();
    ...
    private void DownloadFile(int id)
    {
        try
        {
            ...
            {
                con.Open();
                string sql = "SELECT FileData, FileName, FileType FROM FacultyEvents_tbl WHERE Id=@Id";
